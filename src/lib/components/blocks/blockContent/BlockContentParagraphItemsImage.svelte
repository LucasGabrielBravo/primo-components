  ...
  <ContainerPage>
    <div class="table">
      <div class="box-image">
        ...
        <div class="image-div">
          <img src={image.url} alt={image.alt} class="image" id="from-bot" />
        </div>
      </div>
      <div class="text">
        <span class="paragrafo" id="to-right"> {paragrafo} </span>
        <div class="lista">
...
<style lang="postcss">
  ...
  .table {
    @apply w-full grid grid-cols-1 md:grid-cols-2 gap-12 md:gap-4 overflow-hidden;
  }
  .box-image {
    @apply w-full flex flex-col gap-4;
  }
  .chamada-title {
    @apply border-l-4 border-secondary-500 pl-6 font-bold text-primary-500 text-2xl md:text-4xl max-w-4xl;
  }
  .image-div {
    @apply md:w-[50%] ml-12 my-auto flex;
  }
  .image {
    @apply w-[50%];
  }
